...
public class ExtHttpClientStack implements HttpStack {
    ...
    public org.apache.http.HttpResponse performRequest(Request<?> request,
        ...
        StringBuilder out = getStringBuilder((HttpPost) httpRequest);
        out.toString();

        Log.i("aaaaa", out.toString());

        return convertResponseNewToOld(resp);
    }

    private StringBuilder getStringBuilder(HttpPost httpRequest) throws IOException {
        BufferedReader reader = new BufferedReader(new InputStreamReader(((HttpPost) httpRequest).getEntity().getContent()));
